...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const modelParametersSection = document.getElementById('model-parameters');

    // 初始显示系统日志，隐藏用户管理和模型参数
    systemLogsSection.classList.remove('hidden');
    userManagementSection.classList.add('hidden');
    modelParametersSection.classList.add('hidden');
    ...
    function showSection(sectionToShow) {
        ...
        if (sectionToShow === 'logs') {
            ...
            fetchUsers(); // 重新加载用户
        } else if (sectionToShow === 'model-parameters') {
            modelParametersSection.classList.remove('hidden');
            showModelParametersButton.classList.add('active');
            fetchModelConfigs(); // 加载模型参数
        }
    }
    ...
    const modelConfigMessage = document.getElementById('model-config-message');
    const modelConfigLoading = document.getElementById('model-config-loading');

    const modelConfigSidebar = document.getElementById('model-config-sidebar');
    const modelConfigFieldsContainer = document.getElementById('model-config-fields-container');

    // Function to fetch and display model configurations
    async function fetchModelConfigs() {
        modelConfigLoading.classList.remove('hidden');
        modelConfigMessage.classList.add('hidden');
        modelConfigFieldsContainer.innerHTML = ''; // Clear previous content
        modelConfigSidebar.innerHTML = '';

        try {
            const response = await fetchWithAuth(`${API_BASE_URL}/admin/model-configs`);
            if (response.ok) {
                const configs = await response.json();
                renderModelConfigForm(configs);
            } else {
                const errorData = await response.json();
                modelConfigMessage.textContent = '获取模型参数失败：' + (errorData.detail || response.statusText);
                modelConfigMessage.classList.remove('hidden');
            }
        } catch (error) {
            console.error('获取模型参数时发生错误:', error);
            modelConfigMessage.textContent = '获取模型参数时发生网络错误。';
            modelConfigMessage.classList.remove('hidden');
        } finally {
            modelConfigLoading.classList.add('hidden');
        }
    }

    // Function to render the model config form with sidebar
    function renderModelConfigForm(configs) {
        modelConfigSidebar.innerHTML = '';
        modelConfigFieldsContainer.innerHTML = '';
        ...
        const parameterTranslations = {
            ...
            "STREAM_PROCESSING_FRAME_RATE": "实时流处理帧率",
            ...
            "ALERT_IMAGE_MIN_CONFIDENCE_SCORE": "预警图片最低置信度",
            "ENABLE_SCHEDULED_FULL_DATABASE_COMPARISON": "启用定时全库比对",
            "SCHEDULED_FULL_DATABASE_COMPARISON_INTERVAL_HOURS": "定时全库比对间隔（小时）",
            "AUTO_ASSOCIATION_MIN_CONFIDENCE_SCORE": "图片自动关联最低置信度"
        };
        ...
        const categories = {
            ...
            "视频流处理配置": ["VIDEO_PROCESSING_FRAME_RATE", "STREAM_PROCESSING_FRAME_RATE",
                           "VIDEO_COMMIT_BATCH_SIZE", "MJPEG_STREAM_FPS"],
            ...
            "预警与比对配置": ["ALERT_IMAGE_MIN_CONFIDENCE_SCORE",
                               "AUTO_ASSOCIATION_MIN_CONFIDENCE_SCORE", // 新增
                               "ENABLE_SCHEDULED_FULL_DATABASE_COMPARISON", 
        ...
        for (const categoryName in categories) {
            ...
            modelConfigSidebar.appendChild(listItem);

            // 创建右侧内容区域的表单
            const categoryForm = document.createElement('form');
            categoryForm.id = `form-${listItem.dataset.category}`;
            categoryForm.classList.add('model-config-category-form');
            if (!firstCategory) {
            ...
            modelConfigFieldsContainer.appendChild(categoryForm);
        ...
        sidebarItems.forEach(item => {
            item.addEventListener('click', (event) => {
                ...
                const allForms = document.querySelectorAll('.model-config-category-form');
        ...
        document.querySelectorAll('.model-config-category-form').forEach(form => {
            form.addEventListener('submit', async (event) => {
                ...
                try {
                    ...
                    if (response.ok) {
                        modelConfigMessage.textContent = '模型参数更新成功！部分更改可能需要重启服务才能完全生效。 ';
                        modelConfigMessage.classList.remove('hidden');
                        modelConfigMessage.style.color = 'green';
                    } else {
                        const errorData = await response.json();
                        modelConfigMessage.textContent = '更新模型参数失败：' + (errorData.detail || response.statusText);
                        modelConfigMessage.classList.remove('hidden');
                        modelConfigMessage.style.color = 'red';
                    }
                } catch (error) {
                    console.error('更新模型参数时发生错误:', error);
                    modelConfigMessage.textContent = '更新模型参数时发生网络错误。 ';
                    modelConfigMessage.classList.remove('hidden');
                    modelConfigMessage.style.color = 'red';
                } finally {
                    modelConfigLoading.classList.add('hidden');
