...
from backend.database_conn import engine, Base, SessionLocal, create_tables, get_db, Log # 导入Log模型
from backend.ml_services import ml_logic
from backend import crud
from backend import schemas
from backend.routers import auth_routes, export_routes, person_routes, video_routes, page_routes, admin_routes, stream_routes, image_analysis_routes, human_in_the_loop_routes, followed_person_routes # 确保导入了所有路由模块，包括stream_routes和image_analysis_routes以及human_in_the_loop_routes, followed_person_routes
...
async def startup_event():
    ...
    # 新增：配置 Celery Beat 定时任务
    if settings.ENABLE_SCHEDULED_FULL_DATABASE_COMPARISON:
        logger.info(f"定时全库比对功能已启用，任务将每 {settings.SCHEDULED_FULL_DATABASE_COMPARISON_INTERVAL_HOURS} 小时运行一次。")
        celery_app.conf.beat_schedule = {
            'run-scheduled-full-database-comparison-every-x-hours': {
                'task': 'backend.ml_services.ml_tasks.run_scheduled_full_database_comparison',
                'schedule': datetime.timedelta(hours=settings.SCHEDULED_FULL_DATABASE_COMPARISON_INTERVAL_HOURS),
                'args': (), # 如果任务需要参数，可以在这里提供
            },
        }
        celery_app.conf.timezone = 'Asia/Shanghai' # 确保时区正确
    else:
        logger.info("定时全库比对功能 (ENABLE_SCHEDULED_FULL_DATABASE_COMPARISON) 未启用。")
...
app.mount("/backend/enroll_person_images", StaticFiles(directory=settings.ENROLL_PERSON_IMAGES_DIR), name="enroll_person_images")
...
async def serve_html_pages(page_name: str):
    ...
    file_path = os.path.join(settings.BASE_DIR, "frontend", "static", f"{page_name}.html")
