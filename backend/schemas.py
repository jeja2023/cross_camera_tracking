...
class FollowedPersonBase(BaseModel):
    # person_uuid: str = Field(..., description="被关注的人物UUID") # 移除，改为 individual_id
    individual_id: int = Field(..., description="被关注的 Individual ID")

class FollowedPersonCreate(FollowedPersonBase):
    pass # 创建时只需要 individual_id，follower_id 和 followed_at 由后端自动添加

class FollowedPerson(FollowedPersonBase):
    id: int
    follower_id: int
    person_uuid: Optional[str] = Field(None, description="被关注人物的 UUID") # 重新添加 person_uuid 字段
    followed_at: datetime
    unfollowed_at: Optional[datetime] = None # 取消关注时间

    # 可以选择性地包含被关注人物和关注者的详情
    # followed_person_details: Optional["Person"] = None # 移除，改为 Individual 详情
    followed_individual: Optional["Individual"] = None 
    follower_details: Optional["User"] = None

    # 新增直接显示 Individual 信息的字段，方便前端使用
    individual_name: Optional[str] = Field(None, description="被关注 Individual 的姓名")
    individual_id_card: Optional[str] = Field(None, description="被关注 Individual 的身份证号/ID")
...
class ModelConfig(BaseModel):
    ...
    # 新增：预警功能相关配置
    ALERT_IMAGE_MIN_CONFIDENCE_SCORE: Optional[float] = None

    # 新增：图片解析后自动关联到关注人员的最低置信度
    AUTO_ASSOCIATION_MIN_CONFIDENCE_SCORE: Optional[float] = None

    # 新增：定时全库比对功能开关
    ENABLE_SCHEDULED_FULL_DATABASE_COMPARISON: Optional[bool] = None

    # 新增：定时全库比对间隔时间（小时）
    SCHEDULED_FULL_DATABASE_COMPARISON_INTERVAL_HOURS: Optional[int] = None
...
class PersonSearchRequest(BaseModel):
    ...
    video_uuid: Optional[str] = Field(None, description="可选：限定在某个视频内搜索")
    stream_uuid: Optional[str] = Field(None, description="可选：限定在某个视频流内搜索")
    skip: int = 0
    limit: int = 20
