...
def generate_media_paths(
    ...
    original_filename: Optional[str] = None # 尽管这里不直接用于全帧，但保留以防万一未来需要
) -> Dict[str, str]:
    """
    根据媒体类型、UUID 和人物 UUID 生成存储路径。
    返回一个字典，包含裁剪图、人脸、步态的绝对路径和相对路径。不生成全帧图片路径。
    """
    paths = {}
...
def process_detected_person_data(
    ...
    # 4. 动态获取模型名称
    effective_model_name = model_name if model_name else settings.IMAGE_ANALYSIS_MODEL_NAME
    logger.debug(f"使用模型 '{effective_model_name}' 处理媒体数据。")

    # 2. 保存 Re-ID 裁剪图
    # 裁剪图路径总是需要的，因为我们会提取人物特征
    ...
    cv2.imwrite(crop_image_path_abs, person_crop)
    logger.debug(f"人物裁剪图 (Re-ID) 已保存到: {crop_image_path_abs}")

    # 3. 处理全帧图片路径和保存逻辑
    final_full_frame_image_path_rel = None
    if full_frame_image_path_override is not None:
        # 如果提供了 override 路径 (来自主动注册)，直接使用它作为数据库中的全帧图片路径。
        # 原始图片已由 ml_logic.py 保存，这里无需再次保存，也无需在这里创建 database/full_frames 目录。
        final_full_frame_image_path_rel = full_frame_image_path_override
        logger.debug(f"全帧图片路径已通过 override 提供: {final_full_frame_image_path_rel}，跳过在 media_processing 中生成路径和保存原始帧。")
    else:
        # 如果没有提供 override 路径，则根据 media_type 决定是否生成并保存全帧图片。
        # 这通常适用于视频流分析或直接的图片分析 (非主动注册)。
        ...
        final_full_frame_image_path_rel = settings.get_parsed_image_relative_path(
            base_dir_name="full_frames",
            model_name=effective_model_name,
            analysis_type=media_type,
            uuid=media_uuid
        )
        final_full_frame_image_path_rel = os.path.join(final_full_frame_image_path_rel, full_frame_filename).replace(os.sep, '/')

        os.makedirs(os.path.dirname(full_frame_image_path_abs), exist_ok=True)
        cv2.imwrite(full_frame_image_path_abs, frame)
        logger.debug(f"全帧图片已保存到: {full_frame_image_path_abs}")
    ...
    if yolo_results_obj and yolo_results_obj.keypoints is not None:
        # 遍历所有检测到的人，找到与当前 person_bbox 匹配的关键点
        # 注意：这是一个简化的匹配逻辑，假设 yolo_results_obj.boxes 和 yolo_results_obj.keypoints 的顺序是一致的。
        # 一个更鲁棒的方法是计算 IoU 来匹配框，但通常 YOLOv8 的结果是同步的。
        for i, box in enumerate(yolo_results_obj.boxes.xyxy):
            # 将 bbox 转换为整数元组以便比较
            current_box_tuple = tuple(map(int, box.tolist()))
            if current_box_tuple == person_bbox:
                keypoints = yolo_results_obj.keypoints[i]
                # 将关键点数据转换为 JSON 兼容格式 (例如，列表)
    ...
    # Assuming ml_logic.get_clothing_attributes exists and returns JSON serializable data
    # if ml_logic.has_clothing_attribute_model(): # 假设有一个方法检查模型是否可用
    if hasattr(ml_logic, 'get_clothing_attributes') and callable(getattr(ml_logic, 'get_clothing_attributes')):
        clothing_attributes_data = ml_logic.get_clothing_attributes(frame, person_bbox)
    ...
    person_create_data = schemas.PersonCreate(
        ...
        full_frame_image_path=final_full_frame_image_path_rel, # 使用最终确定的全帧图片路径
        ...
        image_id=image_id_int if media_type == "image" else None,
