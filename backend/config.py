...
class Settings:
    ...
    # Model paths
    DETECTION_MODEL_FILENAME: str = os.path.basename(os.getenv("DETECTION_MODEL_FILENAME")) if os.getenv("DETECTION_MODEL_FILENAME") else ""
    if not DETECTION_MODEL_FILENAME:
        print("错误：目标检测模型文件名 'DETECTION_MODEL_FILENAME' 未在您的 .env 文件中设置。请提供文件名，例如 yolov8n.onnx", file=sys.stderr)
        sys.exit(1)
    DETECTION_MODEL_PATH: str = os.path.join(MODELS_DIR, DETECTION_MODEL_FILENAME)

    REID_MODEL_FILENAME: str = os.path.basename(os.getenv("REID_MODEL_FILENAME")) if os.getenv("REID_MODEL_FILENAME") else ""
    if not REID_MODEL_FILENAME:
        print("错误：Re-ID 模型文件名 'REID_MODEL_FILENAME' 未在您的 .env 文件中设置。请提供文件名，例如 osnet_ibn_x1_0.onnx", file=sys.stderr)
        sys.exit(1)
    REID_MODEL_PATH: str = os.path.join(MODELS_DIR, REID_MODEL_FILENAME)

    # 读取 ACTIVE_REID_MODEL_PATH 环境变量，它预计只包含文件名
    _active_reid_model_filename: str = os.path.basename(os.getenv("ACTIVE_REID_MODEL_PATH")) if os.getenv("ACTIVE_REID_MODEL_PATH") else ""
    if not _active_reid_model_filename:
        print("错误：主动 Re-ID 模型路径 'ACTIVE_REID_MODEL_PATH' 未在您的 .env 文件中设置。请提供文件名，例如 osnet_ibn_x1_0.onnx", file=sys.stderr)
        sys.exit(1)
    # 将模型目录与文件名拼接，得到完整的模型路径
    ACTIVE_REID_MODEL_PATH: str = os.path.join(MODELS_DIR, _active_reid_model_filename)

    # 姿态估计模型 (可选)
    _POSE_MODEL_FILENAME: str = os.path.basename(os.getenv("POSE_MODEL_FILENAME", ""))
    POSE_MODEL_PATH: str = os.path.join(MODELS_DIR, _POSE_MODEL_FILENAME) if _POSE_MODEL_FILENAME else ""

    # 人脸检测模型 (可选)
    _FACE_DETECTION_MODEL_FILENAME: str = os.path.basename(os.getenv("FACE_DETECTION_MODEL_FILENAME", ""))
    FACE_DETECTION_MODEL_PATH: str = os.path.join(MODELS_DIR, _FACE_DETECTION_MODEL_FILENAME) if _FACE_DETECTION_MODEL_FILENAME else ""

    # 人脸识别模型 (可选)
    _FACE_RECOGNITION_MODEL_FILENAME: str = os.path.basename(os.getenv("FACE_RECOGNITION_MODEL_FILENAME", ""))
    FACE_RECOGNITION_MODEL_PATH: str = os.path.join(MODELS_DIR, _FACE_RECOGNITION_MODEL_FILENAME) if _FACE_RECOGNITION_MODEL_FILENAME else ""

    _GAIT_RECOGNITION_MODEL_FILENAME = os.path.basename(os.getenv("GAIT_RECOGNITION_MODEL_FILENAME", ""))
    ...
    _CLOTHING_ATTRIBUTE_MODEL_FILENAME = os.path.basename(os.getenv("CLOTHING_ATTRIBUTE_MODEL_FILENAME", ""))
    ...
    CONFIGURABLE_SETTINGS_KEYS = [
        ...
        "GAIT_SEQUENCE_LENGTH", "STREAM_LOG_INTERVAL_FRAMES", # 将其移到这里，靠近其他数值型配置
                ...
                for key in self.CONFIGURABLE_SETTINGS_KEYS:
                    if hasattr(self, key):
                        ...
                        if key in ["DETECTION_MODEL_FILENAME", "REID_MODEL_FILENAME", 
                                   ...
                                   "CLOTHING_ATTRIBUTE_MODEL_FILENAME", "ACTIVE_REID_MODEL_PATH"]:
                            # 针对 ACTIVE_REID_MODEL_PATH 和其他模型文件，这里应该存储文件名
                            # os.path.basename() 已经在 Settings 初始化时处理，这里直接取属性值即可
                            # 但是为了兼容旧的 .env 文件可能包含完整路径的情况，再次确保只存储文件名
                            configs_to_write[key] = os.path.basename(str(value)) if value is not None else ""
                        elif isinstance(value, int): # 明确处理整数
                            configs_to_write[key] = str(value)
                        elif isinstance(value, float): # 明确处理浮点数
                            configs_to_write[key] = str(value)
                        elif isinstance(value, bool): # 明确处理布尔值
                            configs_to_write[key] = str(value).lower() # 存储为 "true" 或 "false"
                        else:
                            configs_to_write[key] = value # 默认作为字符串存储
                
                # 移除之前对 ACTIVE_REID_MODEL_PATH 的单独特殊处理，现在已合并到上面的循环中
            ...
            for key in self.CONFIGURABLE_SETTINGS_KEYS:
                if key not in existing_db_keys and hasattr(self, key):
                    ...
                    if key in ["DETECTION_MODEL_FILENAME", "REID_MODEL_FILENAME", 
                               ...
                               "CLOTHING_ATTRIBUTE_MODEL_FILENAME", "ACTIVE_REID_MODEL_PATH"]:
                        # 针对模型文件名，这里应该存储文件名
                        new_configs_to_add[key] = os.path.basename(str(value)) if value is not None else ""
                    elif isinstance(value, int): # 明确处理整数
                        new_configs_to_add[key] = str(value)
                    elif isinstance(value, float): # 明确处理浮点数
                        new_configs_to_add[key] = str(value)
                    elif isinstance(value, bool): # 明确处理布尔值
                        new_configs_to_add[key] = str(value).lower() # 存储为 "true" 或 "false"
                    else:
    ...
    def load_from_db(self, db: Session): # 新增方法，从数据库加载配置
        ...
        try:
            ...
            # 创建一个映射，将模型文件名键映射到其对应的完整路径键
            filename_to_path_map = {
                "DETECTION_MODEL_FILENAME": "DETECTION_MODEL_PATH",
                "REID_MODEL_FILENAME": "REID_MODEL_PATH",
                "POSE_MODEL_FILENAME": "POSE_MODEL_PATH",
                "FACE_DETECTION_MODEL_FILENAME": "FACE_DETECTION_MODEL_PATH",
                "FACE_RECOGNITION_MODEL_FILENAME": "FACE_RECOGNITION_MODEL_PATH",
                "GAIT_RECOGNITION_MODEL_FILENAME": "GAIT_RECOGNITION_MODEL_PATH",
                "CLOTHING_ATTRIBUTE_MODEL_FILENAME": "CLOTHING_ATTRIBUTE_MODEL_PATH",
                # "ACTIVE_REID_MODEL_PATH": "ACTIVE_REID_MODEL_PATH" # 这个特殊，它的值已经是相对路径，直接拼接即可
            }
            ...
            for config in db_configs:
                if hasattr(self, config.key) and not config.key.startswith("_") and not callable(getattr(self, config.key)):
                    try:
                        ...
                        # 如果是模型文件名，先更新文件名属性
                        if config.key in filename_to_path_map:
                            setattr(self, config.key, str(config.value) if config.value is not None else "")
                            # 然后根据更新后的文件名，重新计算并设置对应的完整路径属性
                            path_key = filename_to_path_map[config.key]
                            setattr(self, path_key, os.path.join(self.MODELS_DIR, getattr(self, config.key)) if getattr(self, config.key) else "")

                        # 针对 ACTIVE_REID_MODEL_PATH 做特殊处理，因为它是直接存储文件名
                        elif config.key == "ACTIVE_REID_MODEL_PATH":
                            # 从数据库读取的值已经是文件名（或期望是文件名），直接拼接 MODELS_DIR
                            setattr(self, config.key, os.path.join(self.MODELS_DIR, str(config.value)) if config.value else "")
    ...
    STREAM_LOG_INTERVAL_FRAMES_STR: str = os.getenv("STREAM_LOG_INTERVAL_FRAMES")
    if not STREAM_LOG_INTERVAL_FRAMES_STR:
        print("错误：流日志间隔帧数 'STREAM_LOG_INTERVAL_FRAMES' 未在您的 .env 文件中设置。", file=sys.stderr)
        sys.exit(1)
    try:
        STREAM_LOG_INTERVAL_FRAMES: int = int(STREAM_LOG_INTERVAL_FRAMES_STR)
    except ValueError:
        print(f"错误：STREAM_LOG_INTERVAL_FRAMES 的值 '{STREAM_LOG_INTERVAL_FRAMES_STR}' 无效。必须是一个整数。", file=sys.stderr)
        sys.exit(1)
