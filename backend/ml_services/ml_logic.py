...
_last_loaded_reid_model_path: Optional[str] = None # 新增：用于跟踪上次加载的模型路径
...
_face_detection_model_lock = threading.Lock()
_last_loaded_face_detection_model_path: Optional[str] = None # 新增：用于跟踪上次加载的模型路径

# 新增：全局步态识别模型实例和锁
_gait_recognition_session_instance: Optional[ort.InferenceSession] = None
_gait_recognition_model_lock = threading.Lock()
_last_loaded_gait_recognition_model_path: Optional[str] = None # 新增：用于跟踪上次加载的模型路径

# 新增：全局姿态检测模型实例和锁
_detection_pose_model_instance: Optional[YOLO] = None
_detection_pose_model_lock = threading.Lock()
_last_loaded_pose_model_path: Optional[str] = None # 新增：用于跟踪上次加载的模型路径

# 新增：全局衣着属性模型实例和锁
_clothing_attribute_session_instance: Optional[ort.InferenceSession] = None
_clothing_attribute_model_lock = threading.Lock()
_last_loaded_clothing_attribute_model_path: Optional[str] = None # 新增：用于跟踪上次加载的模型路径
...
# 修改：全局姿态估计模型实例和锁
_pose_estimation_model_instance: Optional[YOLO] = None # 重命名
_pose_estimation_model_lock = threading.Lock() # 重命名
_last_loaded_pose_model_path: Optional[str] = None # 保持不变

# 新增：全局主要目标检测模型实例和锁
_main_object_detection_model_instance: Optional[YOLO] = None
_main_object_detection_model_lock = threading.Lock()
_last_loaded_main_object_detection_model_path: Optional[str] = None
...
def get_face_detection_model(db: Session) -> Optional[YOLO]:
    """
    获取人脸检测模型的单例实例。
    """
    global _face_detection_model_instance, _last_loaded_face_detection_model_path
    with _face_detection_model_lock:
        model_path = settings.FACE_DETECTION_MODEL_PATH
        logger.info(f"get_face_detection_model: model_path = {model_path}") # 新增调试日志

        # 如果模型路径为空且当前模型实例存在，则卸载模型
        if not model_path:
            if _face_detection_model_instance is not None:
                logger.info("人脸检测模型路径为空，正在卸载已加载的人脸检测模型。")
                _face_detection_model_instance = None
                _last_loaded_face_detection_model_path = None
            logger.info("人脸检测模型路径未设置，跳过加载人脸检测模型。")
            return None

        # 如果模型路径发生变化，重新加载模型
        if _face_detection_model_instance is not None and model_path != _last_loaded_face_detection_model_path:
            logger.info(f"人脸检测模型路径已从 {_last_loaded_face_detection_model_path} 变更为 {model_path}，正在重新加载。")
            _face_detection_model_instance = None # 强制重新加载
        ...
        if _face_detection_model_instance is None:
            try:
                ...
                _last_loaded_face_detection_model_path = model_path
...
def get_face_recognition_session(db: Session) -> Optional[ort.InferenceSession]:
    """
    获取人脸识别模型的单例实例。
    """
    global _face_recognition_session_instance, _last_loaded_face_recognition_model_path
    with _face_recognition_model_lock:
        model_path = settings.FACE_RECOGNITION_MODEL_PATH
        logger.info(f"get_face_recognition_session: model_path = {model_path}") # 新增调试日志

        if not model_path:
            if _face_recognition_session_instance is not None:
                logger.info("人脸识别模型路径为空，正在卸载已加载的人脸识别模型。")
                _face_recognition_session_instance = None
                _last_loaded_face_recognition_model_path = None
            logger.info("人脸识别模型路径未设置，跳过加载人脸识别模型。")
            return None

        if _face_recognition_session_instance is not None and model_path != _last_loaded_face_recognition_model_path:
            logger.info(f"人脸识别模型路径已从 {_last_loaded_face_recognition_model_path} 变更为 {model_path}，正在重新加载。")
            _face_recognition_session_instance = None
        ...
        if _face_recognition_session_instance is None:
            try:
                ...
                _last_loaded_face_recognition_model_path = model_path
...
def get_gait_recognition_session(db: Session) -> Optional[ort.InferenceSession]:
    ...
    获取步态识别模型的单例实例。
    """
    global _gait_recognition_session_instance, _last_loaded_gait_recognition_model_path
    with _gait_recognition_model_lock:
        model_path = settings.GAIT_RECOGNITION_MODEL_PATH
        logger.info(f"get_gait_recognition_session: model_path = {model_path}") # 新增调试日志

        if not model_path:
            if _gait_recognition_session_instance is not None:
                logger.info("步态识别模型路径为空，正在卸载已加载的步态识别模型。")
                _gait_recognition_session_instance = None
                _last_loaded_gait_recognition_model_path = None
            logger.info("步态识别模型路径未设置，跳过加载步态识别模型。")
            return None

        if _gait_recognition_session_instance is not None and model_path != _last_loaded_gait_recognition_model_path:
            logger.info(f"步态识别模型路径已从 {_last_loaded_gait_recognition_model_path} 变更为 {model_path}，正在重新加载。")
            _gait_recognition_session_instance = None
        ...
        if _gait_recognition_session_instance is None:
            try:
                ...
                _last_loaded_gait_recognition_model_path = model_path
...
def get_pose_estimation_model(db: Session) -> Optional[YOLO]: # 重命名函数
    """
    获取姿态估计模型的单例实例。
    """
    global _pose_estimation_model_instance, _last_loaded_pose_model_path # 更新全局变量名
    with _pose_estimation_model_lock: # 更新锁名
        model_path = settings.POSE_MODEL_PATH
        logger.info(f"get_pose_estimation_model: model_path = {model_path}") # 更新日志

        if not model_path:
            if _pose_estimation_model_instance is not None:
                logger.info("姿态估计模型路径为空，正在卸载已加载的姿态估计模型。")
                _pose_estimation_model_instance = None
                _last_loaded_pose_model_path = None
            logger.info("姿态估计模型路径未设置，跳过加载姿态估计模型。")
            return None

        if _pose_estimation_model_instance is not None and model_path != _last_loaded_pose_model_path:
            logger.info(f"姿态估计模型路径已从 {_last_loaded_pose_model_path} 变更为 {model_path}，正在重新加载。")
            _pose_estimation_model_instance = None

        if _pose_estimation_model_instance is None:
            try:
                if not os.path.exists(model_path):
                    raise FileNotFoundError(f"姿态估计模型未找到: {model_path}")
                logger.info(f"加载姿态估计模型: {model_path}")
                _pose_estimation_model_instance = YOLO(model_path, task='pose')
                _last_loaded_pose_model_path = model_path
                logger.info("姿态估计模型加载成功。")
            except Exception as e:
                logger.error(f"加载姿态估计模型失败: {e}", exc_info=True)
                raise RuntimeError(f"加载姿态估计模型失败: {e}")
        return _pose_estimation_model_instance

def get_main_object_detection_model(db: Session) -> YOLO:
    """
    获取主要目标检测模型的单例实例。
    这个模型是强制性的，如果未设置或加载失败将抛出错误。
    """
    global _main_object_detection_model_instance, _last_loaded_main_object_detection_model_path
    with _main_object_detection_model_lock:
        model_path = settings.DETECTION_MODEL_PATH # 使用 DETECTION_MODEL_PATH
        logger.info(f"get_main_object_detection_model: model_path = {model_path}")

        if not model_path:
            logger.error("主要目标检测模型路径未设置，无法加载核心模型。")
            raise RuntimeError("核心目标检测模型路径未设置。")

        # 如果模型路径发生变化，重新加载模型
        if _main_object_detection_model_instance is not None and model_path != _last_loaded_main_object_detection_model_path:
            logger.info(f"主要目标检测模型路径已从 {_last_loaded_main_object_detection_model_path} 变更为 {model_path}，正在重新加载。")
            _main_object_detection_model_instance = None # 强制重新加载

        if _main_object_detection_model_instance is None:
            try:
                if not os.path.exists(model_path):
                    raise FileNotFoundError(f"主要目标检测模型未找到: {model_path}")
                logger.info(f"加载主要目标检测模型: {model_path}")
                _main_object_detection_model_instance = YOLO(model_path, task='detect') # 假设是detect任务
                _last_loaded_main_object_detection_model_path = model_path
                logger.info("主要目标检测模型加载成功。")
            except Exception as e:
                logger.error(f"加载主要目标检测模型失败: {e}", exc_info=True)
                raise RuntimeError(f"加载主要目标检测模型失败: {e}")
        return _main_object_detection_model_instance

def get_reid_model_session(db: Session) -> ort.InferenceSession:
    """
    获取 Re-ID 模型的单例实例。
    """
    global _reid_session_instance, _last_loaded_reid_model_path # 更新全局变量名
    with _reid_model_lock:
        model_path = settings.ACTIVE_REID_MODEL_PATH
        logger.info(f"get_reid_model_session: model_path = {model_path}") # 新增调试日志

        # Re-ID 模型是强制的，所以这里不需要检查 model_path 是否为空
        # 但是，如果路径发生变化，需要重新加载
        if _reid_session_instance is not None and model_path != _last_loaded_reid_model_path: # 更新实例名
            logger.info(f"Re-ID 模型路径已从 {_last_loaded_reid_model_path} 变更为 {model_path}，正在重新加载。")
            _reid_session_instance = None # 更新实例名

        if _reid_session_instance is None: # 更新实例名
            try:
                if not os.path.exists(model_path):
                    raise FileNotFoundError(f"Re-ID 模型未找到: {model_path}")
                logger.info(f"加载 Re-ID 激活模型: {model_path}")
                _reid_session_instance = ort.InferenceSession(model_path, providers=['CPUExecutionProvider'])
                _last_loaded_reid_model_path = model_path
                logger.info("Re-ID 模型加载成功。")
            except Exception as e:
                logger.error(f"加载 Re-ID 模型失败: {e}", exc_info=True)
                raise RuntimeError(f"加载 Re-ID 模型失败: {e}")
        return _reid_session_instance # 更新实例名

def get_clothing_attribute_session(db: Session) -> Optional[ort.InferenceSession]:
    """
    获取衣着属性模型的单例实例。
    """
    global _clothing_attribute_session_instance, _last_loaded_clothing_attribute_model_path
    with _clothing_attribute_model_lock:
        model_path = settings.CLOTHING_ATTRIBUTE_MODEL_PATH
        logger.info(f"get_clothing_attribute_session: model_path = {model_path}") # 新增调试日志

        if not model_path:
            if _clothing_attribute_session_instance is not None:
                logger.info("衣着属性模型路径为空，正在卸载已加载的衣着属性模型。")
                _clothing_attribute_session_instance = None
                _last_loaded_clothing_attribute_model_path = None
            logger.info("衣着属性模型路径未设置，跳过加载衣着属性模型。")
            return None

        if _clothing_attribute_session_instance is not None and model_path != _last_loaded_clothing_attribute_model_path:
            logger.info(f"衣着属性模型路径已从 {_last_loaded_clothing_attribute_model_path} 变更为 {model_path}，正在重新加载。")
            _clothing_attribute_session_instance = None

        if _clothing_attribute_session_instance is None:
            try:
                if not os.path.exists(model_path):
                    raise FileNotFoundError(f"衣着属性模型未找到: {model_path}")
                logger.info(f"加载衣着属性模型: {model_path}")
                _clothing_attribute_session_instance = ort.InferenceSession(model_path, providers=['CPUExecutionProvider'])
                _last_loaded_clothing_attribute_model_path = model_path
                logger.info("衣着属性模型加载成功。")
            except Exception as e:
                logger.error(f"加载衣着属性模型失败: {e}", exc_info=True)
                raise RuntimeError(f"加载衣着属性模型失败: {e}")
        return _clothing_attribute_session_instance
...
def find_similar_people(db: Session, query_image_path: Optional[str] = None, threshold: float = 0.5, video_uuid: Optional[str] = None, stream_uuid: Optional[str] = None, current_user: schemas.User = None, skip: int = 0, limit: int = 20, query_person_uuids: Optional[List[str]] = None):
    ...
    # Step 2: 如果没有提供 query_person_uuids，则处理 query_image_path (从中提取人物特征作为查询源)
        ...
        try:
            ...
            # 使用主要目标检测模型进行人物检测
            main_detection_model = get_main_object_detection_model(db) # 获取主要检测模型
            detection_results = main_detection_model(original_image, verbose=False) # 使用主要检测模型
            ...
            for r in detection_results:
                if r.boxes is not None:
                    for box_idx, box in enumerate(r.boxes):
                        ...
                        class_name = main_detection_model.names[int(box.cls[0])]
...
def analyze_image(original_image_uuid: str, original_image_full_path: str, original_image_filename: str, db: Session, current_user: schemas.User, perform_realtime_comparison_flag: bool = True, individual_id_to_link: Optional[int] = None, is_enrollment: bool = False):
    ...
    try:
        # 获取主要目标检测模型 (强制)
        main_object_detection_model = get_main_object_detection_model(db) # 获取主要检测模型
        logger.info(f"图片分析：主要目标检测模型加载成功。")

        # 移除旧的姿态检测模型加载逻辑
        # logger.debug(f"尝试加载姿态检测模型: {settings.POSE_MODEL_PATH}")
        # detection_pose_model = get_pose_estimation_model(db)

        # 获取姿态估计模型 (可选)
        pose_estimation_model = get_pose_estimation_model(db)
        if pose_estimation_model: 
            logger.info(f"图片分析：姿态估计模型加载成功。")
        else:
            logger.warning("图片分析：姿态估计模型未加载，相关功能将跳过。")

        # 新增：加载人脸检测和人脸识别模型
        face_detection_model = get_face_detection_model(db)
        face_recognition_session = get_face_recognition_session(db)

        if face_detection_model and face_recognition_session: # 检查人脸模型是否成功加载
            logger.info(f"图片分析：人脸检测模型和人脸识别模型加载成功。")
        else:
            logger.warning("图片分析：人脸检测或人脸识别模型未加载，人脸相关功能将跳过。")
        ...
        # 使用主要目标检测模型进行人物检测
        logger.info("开始进行主要目标检测...")
        results_main_detection = main_object_detection_model(original_image, verbose=False)
        logger.info(f"主要目标检测完成。检测结果数量: {len(results_main_detection)}")

        for r_main in results_main_detection:
            if r_main.boxes is not None and len(r_main.boxes) > 0:
                for i, box in enumerate(r_main.boxes):
                    x1, y1, x2, y2 = map(int, box.xyxy[0])
                    conf = box.conf[0]
                    cls = int(box.cls[0])
                    class_name = main_object_detection_model.names[cls]
                    ...
                    if class_name == 'person' and conf > settings.IMAGE_ANALYSIS_MIN_PERSON_CONFIDENCE:
                        ...
                        try:
                            ...
                            person_data = media_processing.process_detected_person_data(
                                ...
                                yolo_results_obj=r_main, # 传递主要检测结果对象
                                media_type="image",
                                ...
                                model_name="general_detection", # 使用 general_detection 模型名称
                                # 新增：传递人脸检测和人脸识别模型
                                face_detection_model=face_detection_model,
                                face_recognition_session=face_recognition_session,
                                # 新增：传递姿态估计模型
                                pose_estimation_model=pose_estimation_model, # 传递姿态估计模型
        ...
        if track_id not in seen_track_ids:
            ...
            try:
                person_create_data = media_processing.process_detected_person_data(
                    ...
                    gait_recognition_session=gait_recognition_session, # 步态识别模型可以为 None
                    tracklet_gait_buffer=tracklet_gait_buffer,
                    track_id=track_id,
                    model_name="person_reid", # 修改为人体识别
                    face_detection_model=face_detection_model, # 人脸检测模型可以为 None
                    face_recognition_session=face_recognition_session, # 人脸识别模型可以为 None
