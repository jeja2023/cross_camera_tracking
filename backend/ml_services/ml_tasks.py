...
def process_video_for_extraction_task(self, video_path: str, video_id: int):
    ...
    try:
        ...
        logger.info(f"视频任务: settings.POSE_MODEL_PATH = {settings.POSE_MODEL_PATH}")
        logger.info(f"视频任务: settings.FACE_DETECTION_MODEL_PATH = {settings.FACE_DETECTION_MODEL_PATH}")
        logger.info(f"视频任务: settings.FACE_RECOGNITION_MODEL_PATH = {settings.FACE_RECOGNITION_MODEL_PATH}")
        logger.info(f"视频任务: settings.GAIT_RECOGNITION_MODEL_PATH = {settings.GAIT_RECOGNITION_MODEL_PATH}") # 修正属性名
        logger.info(f"视频任务: settings.CLOTHING_ATTRIBUTE_MODEL_PATH = {settings.CLOTHING_ATTRIBUTE_MODEL_PATH}") # 修正属性名
        
        # 初始化模型
        current_detection_model = ml_logic.get_main_object_detection_model(db) # 主要的目标检测模型 (强制)
        reid_model_session = ml_logic.get_reid_model_session(db) # Re-ID 模型 (强制)

        # 可选模型
        pose_estimation_model = ml_logic.get_pose_estimation_model(db) # 获取姿态估计模型
        face_detection_model = ml_logic.get_face_detection_model(db)
        face_recognition_session = ml_logic.get_face_recognition_session(db)
        gait_recognition_session = ml_logic.get_gait_recognition_session(db)
        clothing_attribute_session = ml_logic.get_clothing_attribute_session(db)

        # 检查核心模型是否加载成功，Re-ID 模型和主要检测模型是强制的
        if not current_detection_model:
            logger.error(f"视频 (ID: {video_id}): 主要检测模型 (YOLOv8) 加载失败，任务无法继续。")
            raise Exception("核心 ML 模型 (YOLOv8) 未能成功加载。")

        if not reid_model_session:
            logger.error(f"视频 (ID: {video_id}): Re-ID 模型加载失败，任务无法继续。")
            raise Exception("核心 Re-ID 模型未能成功加载。")
        ...
        while cap.isOpened():
            ...
            if current_detection_model: # 添加对 current_detection_model 的检查
                ...
                results = current_detection_model.track(
                    ...
                    verbose=False,
                    tracker="botsort.yaml" # 新增：明确指定使用 botsort.yaml 配置文件
                )
                    ...
                    if track_id not in session_id_map and track_id not in seen_track_ids: # 确保在当前会话中和所有已保存人物中都是新的
                        ...
                        try:
                            person_obj = media_processing.process_detected_person_data(
                                ...
                                gait_recognition_session=gait_recognition_session, # 如果为None，则传递None
                                clothing_attribute_session=clothing_attribute_session, # 新增
                                # 确保 track_id 也被传递
                                track_id=track_id,
...
def process_live_stream_task(self, stream_id: int, stream_url: str):
    ...
    try:
        ...
        logger.info(f"直播流任务: settings.POSE_MODEL_PATH = {settings.POSE_MODEL_PATH}")
        logger.info(f"直播流任务: settings.FACE_DETECTION_MODEL_PATH = {settings.FACE_DETECTION_MODEL_PATH}")
        logger.info(f"直播流任务: settings.FACE_RECOGNITION_MODEL_PATH = {settings.FACE_RECOGNITION_MODEL_PATH}")
        logger.info(f"直播流任务: settings.GAIT_RECOGNITION_MODEL_PATH = {settings.GAIT_RECOGNITION_MODEL_PATH}") # 修正属性名
        logger.info(f"直播流任务: settings.CLOTHING_ATTRIBUTE_MODEL_PATH = {settings.CLOTHING_ATTRIBUTE_MODEL_PATH}") # 修正属性名
        ...
        # 获取 stream_obj，以便在后续的实时比对逻辑中使用
        stream_obj = backend.crud.get_stream(db, stream_id=stream_id)
        if not stream_obj:
            logger.error(f"视频流 {stream_id}: 无法获取流对象，任务中止。")
            backend.crud.update_stream_status(db, stream_id=stream_id, status="failed")
            db.commit()
            return
        ...
        try:
            # 直接从 ml_logic 获取模型实例
            current_detection_model = ml_logic.get_main_object_detection_model(db) # 主要的目标检测模型 (强制)
            current_reid_session = ml_logic.get_reid_model_session(db) # Re-ID 模型 (强制)
            pose_estimation_model = ml_logic.get_pose_estimation_model(db) # 获取姿态估计模型
            ...
            clothing_attribute_session_instance = ml_logic.get_clothing_attribute_session(db) # 新增

            # 动态设置 TrackerArgs 中的 Re-ID 模型路径和 with_reid 标志 (此段逻辑不再需要)
            ...
            # 移除 tracker_args_instance 和 tracker_stream_instance 的初始化
            # tracker_args_instance = TrackerArgs(
            #     with_reid=False, # 强制禁用 BoT-SORT 内部 Re-ID
            #     model="" # 确保不传递任何模型路径给 BoT-SORT 的内部 Re-ID
            # )
            # tracker_stream_instance = BOTSORT(tracker_args_instance, frame_rate=fps) # 使用实时流的帧率
            # logger.info(f"视频流 {stream_id}: BoT-SORT 跟踪器初始化成功。")

            # 检查核心模型是否加载成功，Re-ID 模型和主要检测模型是强制的
            if not current_detection_model:
                logger.error(f"视频流 (ID: {stream_id}): 主要检测模型 (YOLOv8) 加载失败，任务无法继续。")
                raise Exception("核心 ML 模型 (YOLOv8) 未能成功加载。")

            if not current_reid_session:
                logger.error(f"视频流 (ID: {stream_id}): Re-ID 模型加载失败，任务无法继续。")
                raise Exception("核心 Re-ID 模型未能成功加载。")
        ...
        while cap.isOpened():
            ...
            frame_count += 1 # 增加帧计数
            # 每处理 STREAM_LOG_INTERVAL_FRAMES 帧时记录一次
            if frame_count % settings.STREAM_LOG_INTERVAL_FRAMES == 0:
                elapsed_time = time.time() - start_time
                current_fps = frame_count / elapsed_time if elapsed_time > 0 else 0
                logger.info(f"视频流 (ID: {stream_id}): 已处理 {frame_count} 帧, 耗时 {elapsed_time:.2f} 秒, 速度 {current_fps:.2f} FPS。") # 将 DEBUG 改为 INFO

            # ... (此处省略检测和跟踪逻辑，因为它在原始代码中存在但未在视图中完全显示)
            # 假设检测和跟踪逻辑会更新 annotated_frame
            results_live = None # 初始化 results_live
            if current_detection_model: # 添加对 current_detection_model 的检查
                results_live = current_detection_model.track(
                    source=frame, 
                    persist=True, 
                    tracker="botsort.yaml", # 修正为正确的配置文件名
                    verbose=False,
                    conf=settings.DETECTION_CONFIDENCE_THRESHOLD # 新增：明确指定检测置信度阈值
                )
            else:
                logger.warning(f"视频流 (ID: {stream_id}): 主要检测模型未加载，跳过人物检测和跟踪。")
                    ...
                    if track_id not in seen_track_ids:
                        ...
                        try:
                            person_create_data = media_processing.process_detected_person_data(
                                ...
                                clothing_attribute_session=clothing_attribute_session_instance, # 新增
                                is_enrollment_image=False # 新增：明确标记为非主动注册图片
                            )
        ...
        if cap and cap.isOpened():
            cap.release()
        if out and out.isOpened(): # 确保 out 存在且已打开
            out.release()
        db.close()
