...
@router.get("/followed_persons", summary="Followed Persons Page", include_in_schema=False)
async def followed_persons_page():
    return FileResponse(settings.FOLLOWED_PERSONS_PAGE_PATH)

@router.get("/enrollment_images", summary="Enrollment Images Page", include_in_schema=False)
async def enrollment_images_page():
    return FileResponse(settings.ENROLLMENT_IMAGES_PAGE_PATH)

@router.get("/alert_images", summary="Alert Images Page", include_in_schema=False)
async def alert_images_page():
    return FileResponse(settings.ALERT_IMAGES_PAGE_PATH)
