...
async def export_video_results(
    ...
    for idx, p in enumerate(persons_to_export):
        ...
        # 我们假设所有路径都已标准化为使用正斜杠，因此不再需要单独处理反斜杠路径。
...
async def export_query_results(
    ...
    # 设置表头
    headers = ["序号", "人物ID", "相似度", "来源类型", "来源名称", "来源UUID", "图片缩小图"]
    ws.append(headers)
    # 设置对应列的宽度以容纳图片和文本
    ws.column_dimensions['F'].width = 38 # 来源UUID可能较长，给予足够宽度
    ws.column_dimensions['G'].width = 15 # 图片列的宽度
    logger.info("Excel 表头已设置。")

    # 写入数据
    for idx, result in enumerate(search_results["items"]):
        source_type = "未知"
        source_name = "N/A"
        source_uuid = "N/A"

        if result['stream_uuid']:
            source_type = "视频流"
            source_name = result['stream_name'] or "N/A"
            source_uuid = result['stream_uuid']
        elif result['video_uuid']:
            source_type = "上传视频"
            source_name = result['video_filename'] or "N/A"
            source_uuid = result['video_uuid']

        row_data = [
            idx + 1,
            result['uuid'],
            f"{result['score']:.2f}%", # 格式化相似度为百分比，移除多余的 * 100
            source_type,
            source_name,
            source_uuid
        ]
        ws.append(row_data)
        logger.info(f"正在处理导出结果中的人物: {result['uuid']}，相似度: {result['score']:.2f}%")

        # 添加图片
        # 检查 result['crop_image_path'] 是否存在
        if not result.get('crop_image_path'):
            logger.warning(f"人物 {result['uuid']} 没有 crop_image_path，跳过添加图片。")
            ws.append(["", "", "", "", "", "", "无缩小图"]) # 添加占位符
            continue
        
        # 构建图片绝对路径
        # 移除 result['crop_image_path'] 中重复的 'database/crops/' 前缀
        relative_path_from_crops_root = result['crop_image_path']
        if relative_path_from_crops_root.startswith("database/crops/"):
            # 只保留 'general_detection/video/...' 部分
            relative_path_from_crops_root = relative_path_from_crops_root[len("database/crops/"):]
        # 我们假设所有路径都已标准化为使用正斜杠，因此不再需要单独处理反斜杠路径。
        # elif relative_path_from_crops_root.startswith("database\\crops\\"): # 考虑Windows路径分隔符
        #    relative_path_from_crops_root = relative_path_from_crops_root[len("database\\crops\\"):]

        image_path = os.path.join(settings.DATABASE_CROPS_DIR, relative_path_from_crops_root)
        logger.info(f"尝试为人物 {result['uuid']} 加载图片: {image_path}")
...
async def export_all_persons(
    ...
    file_name = f"all_persons_features_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.xlsx"
...
async def export_person_archives(
    ...
    # 设置表头
    headers = ["序号", "人物UUID", "裁剪图像", "完整帧图像", "来源类型", "来源名称", "来源UUID", "入库时间", "置信度", "是否审核", "是否待再训练"]
    ws.append(headers)
...
async def export_stream_results(
    ...
    for idx, person in enumerate(persons_to_export):
        ...
        temp_path = person.crop_image_path # 移除replace("\\", "/")
