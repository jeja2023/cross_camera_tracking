...
from backend.ml_services.ml_tasks import run_scheduled_full_database_comparison # 导入 run_scheduled_full_database_comparison
...
@router.get("/realtime-comparison-status", response_model=bool, summary="获取实时比对功能状态")
async def get_realtime_comparison_status(
    db: Session = Depends(get_db)
):
    """
    管理员接口：获取实时比对功能 (ENABLE_REALTIME_COMPARISON) 的当前状态。
    """
    return settings.ENABLE_REALTIME_COMPARISON

@router.put("/realtime-comparison-status", response_model=MessageResponse, summary="切换实时比对功能状态")
async def toggle_realtime_comparison_status(
    enable: bool = Query(..., description="设置实时比对功能是否开启"),
    db: Session = Depends(get_db),
    current_user: schemas.User = Depends(auth.get_current_active_admin_user)
):
    """
    管理员接口：切换实时比对功能 (ENABLE_REALTIME_COMPARISON) 的状态。
    """
    try:
        crud.set_system_config(db, key="ENABLE_REALTIME_COMPARISON", value=str(enable))
        settings.reload_from_db(db) # 重新加载设置以反映最新状态
        logger.info(f"管理员 {current_user.username} 已将实时比对功能设置为: {enable}")
        return {"message": f"实时比对功能已设置为: {enable}"}
    except Exception as e:
        logger.error(f"切换实时比对功能状态失败: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="内部服务器错误，无法切换实时比对功能状态。")
...
async def get_model_configs(
    ...
    获取系统当前的模型参数配置。
    """
    # 从 settings 对象中收集所有相关的模型参数
    configs = {
        "DETECTION_MODEL_FILENAME": os.path.basename(settings.DETECTION_MODEL_PATH),
        "REID_MODEL_FILENAME": os.path.basename(settings.REID_MODEL_PATH),
        "ACTIVE_REID_MODEL_PATH": settings.ACTIVE_REID_MODEL_PATH, # 保持为完整路径
        "POSE_MODEL_FILENAME": os.path.basename(settings.POSE_MODEL_PATH),
        "FACE_DETECTION_MODEL_FILENAME": os.path.basename(settings.FACE_DETECTION_MODEL_PATH),
        "FACE_RECOGNITION_MODEL_FILENAME": os.path.basename(settings.FACE_RECOGNITION_MODEL_PATH),
        "GAIT_RECOGNITION_MODEL_FILENAME": os.path.basename(settings.GAIT_RECOGNITION_MODEL_PATH) if settings.GAIT_RECOGNITION_MODEL_PATH else "",
        "CLOTHING_ATTRIBUTE_MODEL_FILENAME": os.path.basename(settings.CLOTHING_ATTRIBUTE_MODEL_PATH) if settings.CLOTHING_ATTRIBUTE_MODEL_PATH else "",
        ...
        "GAIT_SEQUENCE_LENGTH": settings.GAIT_SEQUENCE_LENGTH,

        "HUMAN_REVIEW_CONFIDENCE_THRESHOLD": settings.HUMAN_REVIEW_CONFIDENCE_THRESHOLD,
        "IMAGE_ANALYSIS_MIN_PERSON_CONFIDENCE": settings.IMAGE_ANALYSIS_MIN_PERSON_CONFIDENCE,
        "ENROLLMENT_MIN_PERSON_CONFIDENCE": settings.ENROLLMENT_MIN_PERSON_CONFIDENCE,

        "FACE_DETECTION_CONFIDENCE_THRESHOLD": settings.FACE_DETECTION_CONFIDENCE_THRESHOLD,
        "MIN_FACE_WIDTH": settings.MIN_FACE_WIDTH,
        "MIN_FACE_HEIGHT": settings.MIN_FACE_HEIGHT,

        "TRACKER_PROXIMITY_THRESH": settings.TRACKER_PROXIMITY_THRESH,
        "TRACKER_APPEARANCE_THRESH": settings.TRACKER_APPEARANCE_THRESH,
        "TRACKER_HIGH_THRESH": settings.TRACKER_HIGH_THRESH,
        "TRACKER_LOW_THRESH": settings.TRACKER_LOW_THRESH,
        "TRACKER_NEW_TRACK_THRESH": settings.TRACKER_NEW_TRACK_THRESH,
        "TRACKER_MIN_HITS": settings.TRACKER_MIN_HITS,
        "TRACKER_TRACK_BUFFER": settings.TRACKER_TRACK_BUFFER,

        "VIDEO_PROCESSING_FRAME_RATE": settings.VIDEO_PROCESSING_FRAME_RATE,
        "STREAM_PROCESSING_FRAME_RATE": settings.STREAM_PROCESSING_FRAME_RATE,
        "VIDEO_COMMIT_BATCH_SIZE": settings.VIDEO_COMMIT_BATCH_SIZE,

        "DETECTION_CONFIDENCE_THRESHOLD": settings.DETECTION_CONFIDENCE_THRESHOLD,
        "PERSON_CLASS_ID": settings.PERSON_CLASS_ID,

        "EXCEL_EXPORT_MAX_IMAGES": settings.EXCEL_EXPORT_MAX_IMAGES,
        "EXCEL_EXPORT_IMAGE_SIZE_PX": settings.EXCEL_EXPORT_IMAGE_SIZE_PX,
        "EXCEL_EXPORT_ROW_HEIGHT_PT": settings.EXCEL_EXPORT_ROW_HEIGHT_PT,

        "MJPEG_STREAM_FPS": settings.MJPEG_STREAM_FPS,

        "FAISS_METRIC": settings.FAISS_METRIC,
        "FAISS_SEARCH_K": settings.FAISS_SEARCH_K,

        "REID_TRAIN_BATCH_SIZE": settings.REID_TRAIN_BATCH_SIZE,
        "REID_TRAIN_LEARNING_RATE": settings.REID_TRAIN_LEARNING_RATE,

        # 新增：预警功能相关配置
        "ALERT_IMAGE_MIN_CONFIDENCE_SCORE": settings.ALERT_IMAGE_MIN_CONFIDENCE_SCORE,

        # 新增：图片解析后自动关联到关注人员的最低置信度
        "AUTO_ASSOCIATION_MIN_CONFIDENCE_SCORE": settings.AUTO_ASSOCIATION_MIN_CONFIDENCE_SCORE,

        # 新增：实时比对功能开关 (此为旧的全局开关，已移除)
        # "ENABLE_REALTIME_COMPARISON": settings.ENABLE_REALTIME_COMPARISON,

        # 新增：定时全库比对功能开关
        "ENABLE_SCHEDULED_FULL_DATABASE_COMPARISON": settings.ENABLE_SCHEDULED_FULL_DATABASE_COMPARISON,

        # 新增：定时全库比对间隔时间
        "SCHEDULED_FULL_DATABASE_COMPARISON_INTERVAL_HOURS": settings.SCHEDULED_FULL_DATABASE_COMPARISON_INTERVAL_HOURS,
    }
        ...
        if key in ["DETECTION_MODEL_FILENAME", "REID_MODEL_FILENAME", "POSE_MODEL_FILENAME", 
            ...
            if key == "ACTIVE_REID_MODEL_PATH": # ACTIVE_REID_MODEL_PATH 是一个完整路径
                ...
                configs_to_update[key] = os.path.join(settings.MODELS_DIR, str(value)) if value else ""
                    ...
                    "SCHEDULED_FULL_DATABASE_COMPARISON_INTERVAL_HOURS"]:
            configs_to_update[key] = str(int(value)) # 转换为整数后转字符串
                    ...
                    "ALERT_IMAGE_MIN_CONFIDENCE_SCORE",
                    "AUTO_ASSOCIATION_MIN_CONFIDENCE_SCORE"]:
            configs_to_update[key] = str(float(value)) # 转换为浮点数后转字符串
        elif key in ["ENABLE_SCHEDULED_FULL_DATABASE_COMPARISON"]:
            configs_to_update[key] = str(bool(value)).lower() # 转换为 'true' 或 'false' 字符串
        else:
            configs_to_update[key] = str(value)
...
@router.post("/trigger-scheduled-full-database-comparison", summary="触发定时全库比对任务")
async def trigger_scheduled_full_database_comparison(
    current_user: schemas.User = Depends(auth.get_current_active_admin_user)
):
    """
    管理员接口：手动触发 Celery 后台任务，对所有关注人员的注册图片进行全库比对。
    """
    try:
        task = run_scheduled_full_database_comparison.delay()
        logger.info(f"管理员 {current_user.username} 已手动触发定时全库比对任务 (ID: {task.id})。")
        return {"message": "定时全库比对任务已触发", "task_id": task.id}
    except Exception as e:
        logger.error(f"触发定时全库比对任务失败: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="内部服务器错误，无法触发定时全库比对任务。")
