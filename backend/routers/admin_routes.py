...
@router.get("/system-configs", response_model=ModelConfig, summary="获取系统参数配置") # 修改路径和摘要
    ...
    for key in settings.CONFIGURABLE_SETTINGS_KEYS:
        ...
        if key in ["DETECTION_MODEL_FILENAME", "REID_MODEL_FILENAME", "ACTIVE_REID_MODEL_PATH", 
            ...
            configs[key] = str(value) if value else ""
                    ...
                    "STREAM_LOG_INTERVAL_FRAMES"]: # 新增：流日志间隔帧数
...
@router.put("/system-configs", response_model=MessageResponse, summary="更新系统参数配置") # 修改路径和摘要
async def update_system_configs(
    model_configs: ModelConfig,
    db: Session = Depends(get_db)
):
    """
    更新系统参数配置。这将更新数据库中的配置。
    请注意，部分参数的更改可能需要重启后端服务才能完全生效。
    """
    configs_to_update = {}
    ...
    logger.info("系统参数配置已更新。")
    return {"message": "系统参数配置更新成功。部分更改可能需要重启服务才能完全生效。"}
